...
create a set of thumbnail images that are pushed to the gallery-self/thumbs
...
thumbs_folder = f'{cur_dir}\\imgs\\gallery-self\\thumbs\\'
pages_folder = f'{cur_dir}\\imgs\\gallery-self\\pages\\'
count = 0
...
list_of_imgs = os.listdir(f'{cur_dir}\\imgs\\gallery-self')
...
for img in list_of_imgs:
    #print(DominantColor(f'{cur_dir}\\imgs\\gallery-self\\{img}').rgb)
    #sorted.append([DominantColor(f'{cur_dir}\\imgs\\gallery-self\\{img}').rgb, img])
    sorted.append((DominantColor(f'{cur_dir}\\imgs\\gallery-self\\{img}').rgb, img))
...
for i in range(len(sorted)):
    if '.' in sorted[i][1]:  
        ...
        f = open(f'{cur_dir}\\imgs\\gallery-self\\pages\\{i}_{sorted[i][1]}.html', 'w')
        f.write(
f"""<!DOCTYPE html>
<html lang="en">
<head>
...
<main>
<img id="img-bg" src="https://raw.githubusercontent.com/dual-shock/dual-shock.github.io/main/imgs/gallery-self/thumbs/{i}_{sorted[i][1]}" alt="why no here">   
<div id="fg-container">
<img id="img-fg" src="https://raw.githubusercontent.com/dual-shock/dual-shock.github.io/main/imgs/gallery-self/{sorted[i][1]}" alt="why no here">
</div>
...
<script>
        ...
        image = Image.open(f'{cur_dir}\\imgs\\gallery-self\\{sorted[i][1]}')
        image = ImageOps.exif_transpose(image)  #correct quirk with images 
                                                #not rotating correctly
        image.thumbnail((400,400))
        image.save(f'{cur_dir}\\imgs\\gallery-self\\thumbs\\{i}_{sorted[i][1]}')
